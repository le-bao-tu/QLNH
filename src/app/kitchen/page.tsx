...
import { useBranches, useKitchenTickets } from '@/hooks/useApi'
...
export default function KitchenPage() {
  ...
  const { data: branchs = [] } = useBranches(user?.restaurantId || '')
  const [selectedBranchId, setSelectedBranchId] = useState<string>(branchId)

  const { pageIndex, pageSize, search, setSearch, setPage, paginationParams } = usePagination(24)
  const { data: ticketData, isLoading: loading, refetch: loadTickets } = useKitchenTickets(selectedBranchId, paginationParams)
  ...
  useEffect(() => {
    ...
    console.log('Kitchen Hub connecting to:', hubUrl);

    connection.start()
      .then(() => {
        console.log('Kitchen Hub Connected');
        const kitchenGroup = `kitchen_${branchId.toLowerCase()}`;
        connection.invoke('JoinBranch', kitchenGroup)
          .then(() => console.log(`Kitchen Joined Group: ${kitchenGroup}`))
          .catch(err => console.error('JoinBranch (Kitchen) Error: ', err));
      })
      .catch(err => console.error('Kitchen SignalR Connection Error: ', err));
    ...
    let lastNotifiedOrderId = '';
    let lastNotifyTime = 0;

    connection.on('NewKitchenOrder', (data) => {
      const now = Date.now();
      // Deduplicate if the same order is notified within 5 seconds
      if (data.orderId === lastNotifiedOrderId && (now - lastNotifyTime) < 5000) {
        console.log('[SignalR] Deduplicated Kitchen Notification:', data.orderId);
        return;
      }

      lastNotifiedOrderId = data.orderId;
      lastNotifyTime = now;

      toast.info(`Bàn ${data.tableNumber} vừa gửi món mới vào bếp!`);
      loadTickets();
      playNotificationSound();
    });

    connection.on('NewItems', (data) => {
      // Sometimes universal items event might be useful but we mostly listen to NewKitchenOrder
      loadTickets();
    });
  ...
  }, [branchs, selectedBranchId])

  const tickets = ticketData?.items || (Array.isArray(ticketData) ? ticketData : [])
  const totalCount = ticketData?.totalCount || tickets.length
      ...
      <div className="p-6 bg-slate-900 min-h-screen text-white">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                <span className="flex items-center gap-1 text-xs font-bold text-orange-400 uppercase tracking-widest">
                  <Flame size={14} /> Branch Status: Online
                </span>
                <span className="w-1 h-1 bg-slate-700 rounded-full" />
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{totalCount} Active Tickets</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-6">
            ...
            <div className="relative w-64">
              ...
              <input
                ...
                className="w-full bg-slate-800 border-none rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-orange-500 transition-all font-medium text-white"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="flex bg-slate-800 rounded-xl p-1 border border-slate-700/50">
            {(['all', 'unconfirmed', 'pending', 'cooking', 'ready'] as const).map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
          ...
          <button className="relative w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center hover:bg-slate-700 transition-all border border-slate-700/50">
            ...
            {tickets.some(t => t.status === 'unconfirmed') && (
              <span className="absolute top-2 right-2 w-3 h-3 bg-red-600 rounded-full border-2 border-slate-800 animate-pulse"></span>
            )}
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
              {filteredTickets.map((ticket) => {
                ...
                return (
                  <div
                    key={ticket.id}
                    className={`flex flex-col rounded-3xl overflow-hidden border-2 transition-all ${ticket.status === 'unconfirmed' ? 'border-red-600 bg-red-950/20 shadow-2xl shadow-red-950/40' :
                  ...
                  >
                    {/* Ticket Header */}
                    <div className={`p-5 flex justify-between items-start ${ticket.status === 'unconfirmed' ? 'bg-red-600/10' :
